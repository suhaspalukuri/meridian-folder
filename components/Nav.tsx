  ...
  return (
    <div className="fixed bottom-5 left-1/2 -translate-x-1/2 z-50 pointer-events-none">
      <nav className="pointer-events-auto flex items-center bg-ink/95 backdrop-blur-xl border border-white/[0.07] shadow-[0_8px_40px_rgba(0,0,0,0.45)] rounded-full px-1.5 py-1.5 gap-0.5">
        ...
        <Link
          href="/"
          className="font-serif text-[13px] text-cream/80 hover:text-cream transition-colors px-5 py-2 mr-0.5 border-r border-white/10 whitespace-nowrap"
        >
          TMF
        </Link>
        ...
        {links.map(link => {
          ...
          return (
            <Link
              key={link.href}
              href={link.href}
              className={`flex items-center gap-2 px-4 py-2 rounded-full text-[11px] font-medium tracking-wide transition-all duration-200 whitespace-nowrap ${
                isActive
